...
# HALLwayDEVM - Simplified VM tool for HALLwayDE contributors
# Works on both Arch Linux and NixOS with automatic OS detection

set -e

# Configuration
CACHE_DIR="${XDG_CACHE_HOME:-$HOME/.cache}/hallwaydevm"
BASE_IMAGE="$CACHE_DIR/archbase.qcow2"
SNAPSHOTS_DIR="$CACHE_DIR/snapshots"
HALLWAYDE_REPO="https://github.com/HALLwayDE-Project/HALLwayDE.git"
...
function print_usage() {
    echo "HALLwayDEVM - Simplified VM tool for HALLwayDE contributors"
    echo "Supports: Arch Linux, NixOS"
    echo ""
    echo "Usage: hallwaydevm [OPTIONS] [BRANCH/COMMIT]"
    echo ""
    echo "Arguments:"
    ...
    echo ""
    echo "Examples:"
    echo "  hallwaydevm                  # Run master branch"
    echo "  hallwaydevm --persist        # Run master branch (persistent)"
    echo "  hallwaydevm feature-branch   # Run specific branch"
    echo "  hallwaydevm abc123           # Run specific commit"
    echo "  hallwaydevm --persist dev    # Run dev branch with persistence"
    echo ""
...
function install_all_arch_dependencies() {
    ...
    echo "📦 Installing all HALLwayDEVM dependencies..."
...
function check_deps_only() {
    local os
    os=$(detect_os)
    echo "🔍 Checking HALLwayDEVM dependencies..."
    echo "   Detected OS: $os"
...
function create_hallwayde_snapshot() {
    local ref="${1:-master}"
    local snapshot_name
    snapshot_name=$(get_snapshot_name "$ref")
    local snapshot_path="$SNAPSHOTS_DIR/hallwayde-$snapshot_name.qcow2"
    local qemu_cmd
    qemu_cmd=$(get_qemu_command)
    local python_cmd
    ...
    echo "🔨 Creating HALLwayDE snapshot for '$ref'..."
...
echo "🚀 Setting up HALLwayDE environment for branch/commit: $ref"

# Set root password for convenience
echo "🔐 Setting root password..."
echo -e "hallwaydevm\nhallwaydevm" | sudo passwd root
...
# Clone or update HALLwayDE repository
echo "📥 Setting up HALLwayDE repository..."
cd /home/arch
if [ -d "HALLwayDE" ]; then
    echo "   HALLwayDE directory exists, updating..."
    cd HALLwayDE
    git fetch origin
    git reset --hard HEAD  # Reset any local changes
else
    echo "   Cloning HALLwayDE repository..."
    git clone "$HALLWAYDE_REPO" HALLwayDE
    cd HALLwayDE
fi
...
echo ""
echo "🎨 HALLwayDE repository ready!"

# Check if HALLwayDE is already installed
if [ -f "/home/arch/.config/hypr/hyprland.conf" ] && [ -f "/home/arch/.config/hallwayde/hallwayde.conf" ]; then
    echo "⚠️  HALLwayDE appears to already be installed."
    echo "   Configuration files found. Skipping installation."
    echo "   If you want to reinstall, remove ~/.config/hypr and ~/.config/hallwayde first."
else
    echo "🚀 Starting HALLwayDE installation..."
    cd /home/arch/HALLwayDE/Scripts
    ./install.sh
    echo "✅ HALLwayDE installation complete!"
fi
...
SETUP_EOF
    ...
    echo ""
    echo "🖥️  Starting VM for HALLwayDE installation..."
    ...
    cd "$CACHE_DIR"
    # TODO: feat(hallwaydevm) migrate from the python http server to a pure ssh solution, no setup script needed
    ...
    echo "✅ Snapshot created: hallwayde-$snapshot_name"
    echo "🚀 You can now run: hallwaydevm $ref"
}
...
function run_vm() {
    ...
    echo "🚀 Starting HALLwayDE VM (branch/commit: $ref)..."
...
function list_snapshots() {
    echo "📸 Available HALLwayDE snapshots:"
    if [ -d "$SNAPSHOTS_DIR" ]; then
        find "$SNAPSHOTS_DIR" -name "hallwayde-*.qcow2" -exec basename {} \; | \
            sed 's/^hallwayde-//' | sed 's/\.qcow2$//' | sort
    else
...
function clean_cache() {
    echo "🧹 Cleaning HALLwayDEVM cache..."
    rm -rf "$CACHE_DIR"
    echo "✅ Cache cleaned"
}
