...
- [HydeVM - Simplified VM Tool for HyDE Contributors](#hallwaydevm---simplified-vm-tool-for-hyde-contributors)
...
When you run a new branch/commit for the first time, hallwaydevm will:
...
hallwaydevm feature-branch
hallwaydevm abc123def

# Run with persistence (changes will be saved)
hallwaydevm --persist
hallwaydevm --persist dev-branch
...
# Check dependencies
hallwaydevm --check-deps

# Install dependencies (Arch only)
hallwaydevm --install-deps
```
...
VM_MEMORY=8G VM_CPUS=4 hallwaydevm

# Set extra QEMU arguments
VM_EXTRA_ARGS="-display vnc=:1" hallwaydevm

# Override QEMU command entirely, provided $VM_DISK will be substituted with the actual disk image
VM_QEMU_OVERRIDE="qemu-system-x86_64 -m 4G -smp 2 -enable-kvm -drive file=\$VM_DISK,format=qcow2,if=virtio -device virtio-vga -display gtk" hallwaydevm
```
...
- **Cache Directory**: Uses XDG Base Directory specification (`$XDG_CACHE_HOME/hallwaydevm/`)
- **Snapshots**: Stored in `$XDG_CACHE_HOME/hallwaydevm/snapshots/` (typically `~/.cache/hallwaydevm/snapshots/`)
- **Base Image**: Cached in `$XDG_CACHE_HOME/hallwaydevm/archbase.qcow2` (typically `~/.cache/hallwaydevm/archbase.qcow2`)
...
```bash
hallwaydevm --clean  # Remove all cached data from $XDG_CACHE_HOME/hallwaydevm/
```
...
VM_EXTRA_ARGS="-device virtio-vga -display gtk,gl=off" hallwaydevm
```
...
VM_EXTRA_ARGS="-device VGA -display gtk,gl=off" hallwaydevm
```
...
VM_QEMU_OVERRIDE="qemu-system-x86_64 -m 4G -smp 2 -enable-kvm -cpu host -machine q35 -device intel-iommu -drive file=\$VM_DISK,format=qcow2,if=virtio -device virtio-vga-gl -display gtk,gl=on,grab-on-hover=on -usb -device usb-tablet -device ich9-intel-hda -device hda-output -vga none" hallwaydevm
