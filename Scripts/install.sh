...
HALLWAYDE_LOG="$(date +'%y%m%d_%Hh%Mm%Ss')"
export flg_DryRun flg_Nvidia flg_Shell flg_Install flg_ThemeInstall HALLWAYDE_LOG
...
EOF
	...
	trap 'mv "${scrDir}/install_pkg.lst" "${cacheDir}/logs/${HALLWAYDE_LOG}/install_pkg.lst"' EXIT
	...
	if ! chk_list "aurhlpr" "${aurList[@]}"; then
		...
		*)
		...
		if [[ -z "$getAur" ]]; then
			print_log -sec "AUR" -crit "No AUR helper found..." "Log file at ${cacheDir}/logs/${HALLWAYDE_LOG}"
			exit 1
		fi
	fi
	...
	if ! chk_list "myShell" "${shlList[@]}"; then
		...
		*)
		...
		if [[ -z "$myShell" ]]; then
			print_log -sec "shell" -crit "No shell found..." "Log file at ${cacheDir}/logs/${HALLWAYDE_LOG}"
			exit 1
		else
	...
	if ! grep -q "^#user packages" "${scrDir}/install_pkg.lst"; then
		print_log -sec "pkg" -crit "No user packages found..." "Log file at ${cacheDir}/logs/${HALLWAYDE_LOG}/install.sh"
		exit 1
	fi
...
EOF
	...
	if [ "${flg_DryRun}" -ne 1 ]; then
		export PATH="$HOME/.local/lib/hallwayde:$HOME/.local/bin:${PATH}"
		"$HOME/.local/lib/hallwayde/wallpaper/cache.sh" commence -t ""
		"$HOME/.local/lib/hallwayde/theme.switch.sh" -q || true
		"$HOME/.local/lib/hallwayde/waybar.py" --update || true
...
print_log -b "Log" " :: " -y "View logs at ${cacheDir}/logs/${HALLWAYDE_LOG}"
if [ $flg_Install -eq 1 ] ||
	[ $flg_Restore -eq 1 ] ||
	[ $flg_Service -eq 1 ] &&
	...
	print_log -stat "HALLwayDE" "It is not recommended to use newly installed or upgraded HALLwayDE without rebooting the system. Do you want to reboot the system? (y/N)"
	read -r answer
