...
EOF
	...
	if [ "${flg_DryRun}" -eq 1 ]; then
		# Reports what the real run below would move; moves nothing.
		...
		# Before the deploy, so the preserve action below finds hyprland.lua
		# gone and puts HyDE's template in its place (#2132). Only once deez is
		# known to be there: an exit between here and the deploy would leave no
		# hyprland.lua at all. A failure only warns: the rest of the restore
		...
		# Core carries hyprland.lua. If it did not put the template where the
		# retired file was, that file goes back rather than leaving none.
