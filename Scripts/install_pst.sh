...
if pkg_installed sddm; then
    ...
    if [ ! -f /etc/sddm.conf.d/backup_the_doorwayde_project.conf ] || [ "${DOORWAYDE_INSTALL_SDDM}" = true ]; then
        ...
        *) sddmtheme="Corners" ;;
        ...
        if [[ ${flg_DryRun} -ne 1 ]]; then
            ...
            sudo touch /etc/sddm.conf.d/the_doorwayde_project.conf
            sudo cp /etc/sddm.conf.d/the_doorwayde_project.conf /etc/sddm.conf.d/backup_the_doorwayde_project.conf
            sudo cp /usr/share/sddm/themes/${sddmtheme}/the_doorwayde_project.conf /etc/sddm.conf.d/
        fi
