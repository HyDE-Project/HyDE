...
retire_autogenerated_hypr_config() {
    ...
    # Hyprland's exact header line, and only in the file's leading comment
    # block: after the v26.8.1 loader, blank lines and comments, but before
    # any code. The same line further down, e.g. pasted into a user's own
    # config, does not count.
    awk '
        /^if not hyde then$/ { loader = 1; next }
        loader { if (/^end$/) loader = 0; next }
        /^-- AUTOGENERATED HYPRLAND CONFIG\. *--$/ { found = 1; exit }
        /^[[:space:]]*(--.*)?$/ { next }
        { exit }
        END { exit !found }
    ' "${target}" || return 0
