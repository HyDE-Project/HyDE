...
- [HydeVM - Simplified VM Tool for HyDE Contributors](#doorwaydevm---simplified-vm-tool-for-hyde-contributors)
...
When you run a new branch/commit for the first time, doorwaydevm will:
...
doorwaydevm feature-branch
doorwaydevm abc123def

# Run with persistence (changes will be saved)
doorwaydevm --persist
doorwaydevm --persist dev-branch
...
# Check dependencies
doorwaydevm --check-deps

# Install dependencies (Arch only)
doorwaydevm --install-deps
```
...
VM_MEMORY=8G VM_CPUS=4 doorwaydevm

# Set extra QEMU arguments
VM_EXTRA_ARGS="-display vnc=:1" doorwaydevm

# Override QEMU command entirely, provided $VM_DISK will be substituted with the actual disk image
VM_QEMU_OVERRIDE="qemu-system-x86_64 -m 4G -smp 2 -enable-kvm -drive file=\$VM_DISK,format=qcow2,if=virtio -device virtio-vga -display gtk" doorwaydevm
```
...
- **Cache Directory**: Uses XDG Base Directory specification (`$XDG_CACHE_HOME/doorwaydevm/`)
- **Snapshots**: Stored in `$XDG_CACHE_HOME/doorwaydevm/snapshots/` (typically `~/.cache/doorwaydevm/snapshots/`)
- **Base Image**: Cached in `$XDG_CACHE_HOME/doorwaydevm/archbase.qcow2` (typically `~/.cache/doorwaydevm/archbase.qcow2`)
...
```bash
doorwaydevm --clean  # Remove all cached data from $XDG_CACHE_HOME/doorwaydevm/
```
...
VM_EXTRA_ARGS="-device virtio-vga -display gtk,gl=off" doorwaydevm
```
...
VM_EXTRA_ARGS="-device VGA -display gtk,gl=off" doorwaydevm
```
...
VM_QEMU_OVERRIDE="qemu-system-x86_64 -m 4G -smp 2 -enable-kvm -cpu host -machine q35 -device intel-iommu -drive file=\$VM_DISK,format=qcow2,if=virtio -device virtio-vga-gl -display gtk,gl=on,grab-on-hover=on -usb -device usb-tablet -device ich9-intel-hda -device hda-output -vga none" doorwaydevm
