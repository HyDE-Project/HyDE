...
if [ "$flg_ThemeInstall" -eq 1 ]; then
    ...
    while IFS='"' read -r _ themeName _ themeRepo; do
        ...
        themePath="${confDir}/doorwayde/themes/${themeName}"
        ...
        if [ "${THEME_IMPORT_ASYNC}" -ne 1 ] && [ "${flg_DryRun}" -ne 1 ]; then
            if ! "${cloneDir}/Configs/.local/lib/doorwayde/theme.patch.sh" "${themeName}" "${themeRepo}" "--skipcaching" "false"; then
    ...
    if [ "${THEME_IMPORT_ASYNC}" -eq 1 ]; then
        set +e
        parallel --bar --link "\"${cloneDir}/Configs/.local/lib/doorwayde/theme.patch.sh\"" "{1}" "{2}" "{3}" "{4}" ::: "${themeNameQ[@]}" ::: "${themeRepoQ[@]}" ::: "--skipcaching" ::: "false"
        set -e
    fi
    print_log -y "Run \'doorwayde-shell reload\' if themes look broken"
fi
