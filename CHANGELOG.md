...
- Waybar: the personal LAGC look (transparent floating islands, compact spacing, icon/text breathing room) lives entirely in `user-style.css`, which HyDE always imports last so it applies on top of any layout. No custom layout is shipped — pick any HyDE layout and the LAGC styling still applies. The reusable `brightness-panel` module stays available to add to whichever layout you use.
