...
- Waybar: add the fork's own `lagc` layout (compact islands with the temperature modules cpuinfo/gpuinfo and the DDC/CI brightness panel), selected by default in post-install via `hyde-shell waybar --set lagc`. Colors/island styling still come from the active theme's waybar.theme, so the layout works with any theme and can be cycled with SUPER+ALT+UP/DOWN.
- Waybar: the LAGC look (floating islands, borders, spacing, icon/text breathing room) now lives in each LAGC theme's `waybar.theme` (rendered into the theme-scoped `theme.css`), not in the global `user-style.css`. So the LAGC styling applies only while an LAGC theme is active; switching to an upstream HyDE theme restores that theme's own Waybar look. `user-style.css` is left empty for the end user (upstream cascade). The reusable `brightness-panel` module stays available to add to whichever layout you use.
