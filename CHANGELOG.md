...
- Install: extend #2042's locale adaptation to SDDM and hyprlock clocks. The SDDM greeter themes and hyprlock presets hardcoded an English 12h time/date format that ignored the system locale; `restore_locale.sh` now switches hyprlock's presets to 24h alongside the waybar clock, and the SDDM theme's `HourFormat`/`DateFormat` are blanked at theme-install time so Qt's locale-aware default applies instead
