...
### Fixed
- Theme: a `[hyprland] gtk_theme` (or icon, cursor, font, ...) override in `config.toml` now reaches gsettings too (#2132). `color/hypr.sh` read only the theme's own `hypr.theme` for the Lua ui state that `color/dconf.lua` writes into gsettings on every wallbash run, so the override changed the GTK4 symlink but GTK3 apps that follow gsettings (Firefox, blueman) stayed on the theme's GTK theme. It now applies the state `hyprland.conf` on top of the theme, like `theme.switch.sh`
