...
- Waybar: the LAGC look (floating islands, borders, spacing, icon/text breathing room) now lives in each LAGC theme's `waybar.theme` (rendered into the theme-scoped `theme.css`), not in the global `user-style.css`. So the LAGC styling applies only while an LAGC theme is active; switching to an upstream HyDE theme restores that theme's own Waybar look. `user-style.css` is left empty for the end user (upstream cascade), and no custom layout is shipped — pick any layout and the active theme drives the look. The reusable `brightness-panel` module stays available to add to whichever layout you use.
