...
### Fixed
- Waybar: `hyde-shell waybar --next`/`--prev` (`SUPER+ALT+Up/Down`) no longer crashes with "ValueError: '…/layouts/backup/…' is not in list" (#2133). Layout backups live in `layouts/backup/` inside a layout directory and were picked up as layouts; since `backup/…` sorts before most names, the fallback that takes the first layout or matches `config.jsonc` by hash could settle on a backup. Backups are no longer treated as layouts, and cycling starts from the first or last layout when a backup applied from the backup menu is the current one
