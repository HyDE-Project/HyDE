...
USAGE() {
  cat <<"USAGE"
	...
	Options:
		...
		c     Select area or window with frozen screen and copy to clipboard (no annotation)
...
copy_screenshot() {
  ...
  shift
  local extra_args=("$@")
  if "$LIB_DIR/hyde/screenshot/grimblast" "${extra_args[@]}" copy "$mode"; then
    send_notifs -r 9 -a "HyDE Alert" "Screenshot" "Copied to clipboard"
  else
    send_notifs -a "HyDE Alert" "Screenshot Error" "Failed to take screenshot"
    return 1
  fi
