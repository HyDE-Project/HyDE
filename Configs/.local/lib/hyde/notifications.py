...
def format_history(history):
    ...
    alt = "none"
    tooltip_click = []
    tooltip_click.append(f"󰎟 {t('notification.title', 'Notifications')}")
    tooltip_click.append(f"󰳽 {t('notification.history_pop', 'scroll-down:  history pop')}")
    tooltip_click.append(f"󰳽 {t('notification.toggle_dnd', 'click-left:  Enable & Disable DND')}")
    tooltip_click.append(f"󰳽 {t('notification.clear_history', 'click-middle: 󰛌 clear history')}")
    tooltip_click.append(f"󰳽 {t('notification.close_all', 'click-right: 󱄊 close all')}")
