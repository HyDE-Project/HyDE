...
cacheDir="$HALLWAYDE_CACHE_HOME"
dcol_mode="${dcol_mode:-dark}"
[ -f "$cacheDir/wall.dcol" ] && source "$cacheDir/wall.dcol"
enableWallDcol="${enableWallDcol:-1}"
if [ "$enableWallDcol" -eq 0 ]; then
    HALLWAYDE_THEME_DIR="${HALLWAYDE_THEME_DIR:-$confDir/hallwayde/themes/$HALLWAYDE_THEME}"
    dcol_mode=$(get_hyprConf "COLOR_SCHEME")
    dcol_mode=${dcol_mode#prefer-}
    [ -f "$HALLWAYDE_THEME_DIR/theme.dcol" ] && source "$HALLWAYDE_THEME_DIR/theme.dcol"
fi
{
