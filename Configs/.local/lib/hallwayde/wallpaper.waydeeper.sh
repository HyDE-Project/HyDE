...
lockDir="${XDG_RUNTIME_DIR:-/run/user/$(id -u)}/hallwayde"
mkdir -p "$lockDir"
...
selected_wall="${1:-"$HALLWAYDE_CACHE_HOME/wall.set"}"
...
if [ "$is_video" -eq 1 ]; then
    ...
    cached_thumb="$HALLWAYDE_CACHE_HOME/wallpapers/$(${hashMech:-sha1sum} "$selected_wall" | cut -d' ' -f1).png"
    ...
    if [ ! -f "$cached_thumb" ]; then
        ...
        notify-send -a "HALLwayDE Alert" "ERROR: failed to extract thumbnail from video"
        exit 1
    fi
...
if ! command -v waydeeper &>/dev/null; then
    ...
    notify-send -a "HALLwayDE Alert" "ERROR: waydeeper is not installed"
    exit 1
fi
...
if [ ! -d "$XDG_DATA_HOME/waydeeper/models/inpaint" ] && [ ! -d "$HOME/.local/share/waydeeper/models/inpaint" ]; then
    ...
    if ! waydeeper download-model inpaint; then
        ...
        notify-send -a "HALLwayDE Alert" "ERROR: failed to download waydeeper inpaint model"
        exit 1
    fi
fi
...
if [ -n "$WALLPAPER_WAYDEEPER_MODEL" ]; then
    ...
    if [ ! -f "$XDG_DATA_HOME/waydeeper/models/$WALLPAPER_WAYDEEPER_MODEL.onnx" ] && [ ! -f "$HOME/.local/share/waydeeper/models/$WALLPAPER_WAYDEEPER_MODEL.onnx" ]; then
        ...
        if ! waydeeper download-model $WALLPAPER_WAYDEEPER_MODEL; then
            ...
            notify-send -a "HALLwayDE Alert" "ERROR: failed to download waydeeper $WALLPAPER_WAYDEEPER_MODEL model"
            exit 1
        fi
    fi
fi
