#!/usr/bin/env python3
"""
HALLwayDE's LuaJIT environment manager
...
HALLWAYDE_STATE_DIR = os.path.join(XDG_STATE_HOME, "hallwayde")
LUA_ENV_DIR = os.path.join(XDG_STATE_HOME, "hallwayde", "lua_env")
...
ROCKS_SNAPSHOT = os.path.join(HALLWAYDE_STATE_DIR, "luarocks_env.json")
...
def ensure_state_dir():
    os.makedirs(HALLWAYDE_STATE_DIR, exist_ok=True)
...
def main():
    parser = argparse.ArgumentParser(
        prog="lua_env",
        usage="lua_env.py [command] <options>",
        description="HALLwayDE's LuaJIT environment manager",
