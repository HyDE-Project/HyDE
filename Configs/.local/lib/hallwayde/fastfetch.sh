...
[ -f "$HALLWAYDE_STATE_HOME/staterc" ] && source "$HALLWAYDE_STATE_HOME/staterc"
[ -f "/etc/os-release" ] && source "/etc/os-release"
confDir="${XDG_CONFIG_HOME:-$HOME/.config}"
iconDir="${XDG_DATA_HOME:-$HOME/.local/share}/icons"
cacheDir="${XDG_CACHE_HOME:-$HOME/.cache}/hallwayde"
image_dirs=()
hallwayde_distro_logo=$iconDir/Wallbash-Icon/distro/$LOGO
case $1 in
    logo)
        random() {
            (   
                ...
                if [ -n "$HALLWAYDE_THEME" ] && [ -d "$confDir/hallwayde/themes/$HALLWAYDE_THEME/logo" ]; then
                    image_dirs+=("$confDir/hallwayde/themes/$HALLWAYDE_THEME/logo")
                fi
                [ -f "$hallwayde_distro_logo" ] && echo "$hallwayde_distro_logo"
...
options:
  ...
  --theme   Display a logo inside the hallwayde theme directory
            ...
            for arg in "$@"; do
                case $arg in
                    ...
                    --prof)
                        ...
                        ;;
                    --os)
                        [ -f "$hallwayde_distro_logo" ] && image_dirs+=("$hallwayde_distro_logo")
                        ;;
                    --local)
                        image_dirs+=("$confDir/fastfetch/logo")
                        ;;
                    --wall)
                        ...
                        ;;
                    --theme) if
                        [ -n "$HALLWAYDE_THEME" ] && [ -d "$confDir/hallwayde/themes/$HALLWAYDE_THEME/logo" ]
                    then
                        image_dirs+=("$confDir/hallwayde/themes/$HALLWAYDE_THEME/logo")
                    fi ;;
                esac
