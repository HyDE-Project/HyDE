...
load_dconf_kdeglobals() {
    source "$LIB_DIR/hallwayde/color/hypr.sh"
    source "$LIB_DIR/hallwayde/color/dconf.sh"
...
fn_wallbash() {
    ...
    [ -f "$HALLWAYDE_STATE_HOME/state" ] && source "$HALLWAYDE_STATE_HOME/state"
    [ -f "$HALLWAYDE_STATE_HOME/config" ] && source "$HALLWAYDE_STATE_HOME/config"
    ...
    export WALLBASH_SCRIPTS confDir hallwaydeConfDir cacheDir thmbDir dcolDir iconsDir themesDir fontsDir wallbashDirs enableWallDcol HALLWAYDE_THEME_DIR HALLWAYDE_THEME GTK_ICON GTK_THEME CURSOR_THEME
...
set -a
source "$dcol_file"
if [ -f "$HALLWAYDE_THEME_DIR/theme.dcol" ] && [ "$enableWallDcol" -eq 0 ]; then
    source "$HALLWAYDE_THEME_DIR/theme.dcol"
    print_log -sec "wallbash" -stat "override" "dominant colors from $HALLWAYDE_THEME theme"
    print_log -sec "wallbash" -stat " NOTE" "Remove \"$HALLWAYDE_THEME_DIR/theme.dcol\" to use wallpaper dominant colors"
fi
...
if [ "$enableWallDcol" -eq 0 ] && [[ $reload_flag -eq 1 ]]; then
    print_log -sec "wallbash" -stat "apply $dcol_mode colors" "$HALLWAYDE_THEME theme"
    mapfile -d '' -t deployList < <(find -H "$HALLWAYDE_THEME_DIR" -type f -name "*.theme" -print0)
    while read -r pKey; do
        fKey="$(find -H "$HALLWAYDE_THEME_DIR" -type f -name "$(basename "${pKey%.dcol}.theme")")"
