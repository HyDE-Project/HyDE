...
kmenuDesk="$kmenuPath/hallwaydewallpaper.desktop"
tgtPath="$(dirname "$HALLWAYDE_THEME_DIR")"
get_themes
...
if [ ! -z "$setTheme" ] && [ ! -z "$setWall" ]; then
    ...
    if [[ ${wallHash[@]} == *"$inwallHash"* ]]; then
        notify-send -a "HALLwayDE Notify" -i "$thmbDir/$inwallHash.sqre" "Error" "Hash matched in $setTheme"
        exit 0
    fi
    ...
    notify-send -a "HALLwayDE Alert" -i "$thmbDir/$inwallHash.sqre" "Wallpaper set in $setTheme"
else
