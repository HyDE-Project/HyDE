...
selector_menu() {
    ...
    if [ -n "$RofiSel" ]; then
        ...
        notify-send -a "HALLwayDE Alert" -i "$rofiAssetDir/theme_style_$selectedStyle.png" "Style $selectedStyle applied..."
...
rofiSel=$(
    ...
    while [ $i -lt ${#thmList[@]} ]; do
        ...
        -select "$HALLWAYDE_THEME"
)
if [ -n "$rofiSel" ]; then
    "$LIB_DIR/hallwayde/theme.switch.sh" -s "$rofiSel"
    notify-send -a "HALLwayDE Alert" -i "$iconsDir/Wallbash-Icon/hallwayde.png" " $rofiSel"
fi
