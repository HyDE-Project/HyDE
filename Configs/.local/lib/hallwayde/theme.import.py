...
REPO_URL = "https://github.com/HyDE-Project/hallwayde-gallery.git"
CLONE_DIR = os.path.join(
    os.getenv("XDG_CACHE_HOME", os.path.expanduser("~/.cache")),
    "hallwayde/gallery-database",
)
JSON_DATA = None
...
def fetch_data():
    global JSON_DATA
    json_file_path = os.path.join(CLONE_DIR, "hallwayde-themes.json")
...
def get_theme_preview(theme):
    ...
    if theme == "[CONFIRM]":
        ...
        preview_text += (
            "👋 Let's HALLwayDErate your system with more cool themes!\n\n"
            "  [TAB] to mark a theme\n"
...
def fzf_menu():
    try:
        ...
        subprocess.run(["hallwayde-shell", "reload"], check=True)
...
def fetch_all_themes():
    ...
    themes_dir = os.path.join(config_home, "hallwayde/themes")
...
def main():
    parser = argparse.ArgumentParser(
        description="Imports themes from hallwayde-gallery repository",
        epilog="Env:\n"
    ...
    parser.add_argument(
        ...
        type=str,
        metavar="THEME",
        help="Fetch and update a specific theme by name (`all` to fetch all themes located in 'xdg_config/hallwayde/themes')",
    )
