...
export HALLWAYDE_CONFIG_HOME="$XDG_CONFIG_HOME/hallwayde"
export HALLWAYDE_DATA_HOME="$XDG_DATA_HOME/hallwayde"
export HALLWAYDE_CACHE_HOME="$XDG_CACHE_HOME/hallwayde"
export HALLWAYDE_STATE_HOME="$XDG_STATE_HOME/hallwayde"
export HALLWAYDE_RUNTIME_DIR="$XDG_RUNTIME_DIR/hallwayde"
export ICONS_DIR="$XDG_DATA_HOME/icons"
export FONTS_DIR="$XDG_DATA_HOME/fonts"
export THEMES_DIR="$XDG_DATA_HOME/themes"
export scrDir="${LIB_DIR:-$HOME/.local/lib}/hallwayde"
export confDir="${XDG_CONFIG_HOME:-$HOME/.config}"
export hallwaydeConfDir="$HALLWAYDE_CONFIG_HOME"
export cacheDir="$HALLWAYDE_CACHE_HOME"
export thmbDir="$HALLWAYDE_CACHE_HOME/thumbs"
export dcolDir="$HALLWAYDE_CACHE_HOME/dcols"
...
get_hashmap() {
    ...
    if [ -z "${#wallList[@]}" ] || [[ ${#wallList[@]} -eq 0 ]]; then
        if [[ $skipStrays -eq 1 ]]; then
            ...
            [ -n "$no_notify" ] && notify-send -a "HALLwayDE Alert" "WARNING: No compatible wallpapers found in: ${no_wallpapers[*]}"
            exit 1
        fi
    fi
...
get_themes() {
    ...
    while read -r thmDir; do
        ...
        thmListS+=("${thmDir##*/}")
    done < <(find -H "$HALLWAYDE_CONFIG_HOME/themes" -mindepth 1 -maxdepth 1 -type d)
...
export_hallwayde_config() {
    local user_conf_state="$XDG_STATE_HOME/hallwayde/staterc"
    local user_conf="$XDG_STATE_HOME/hallwayde/config"
...
if [ -z "$HALLWAYDE_THEME" ] || [ ! -d "$HALLWAYDE_CONFIG_HOME/themes/$HALLWAYDE_THEME" ]; then
    get_themes
    HALLWAYDE_THEME="${thmList[0]}"
fi
HALLWAYDE_THEME_DIR="$HALLWAYDE_CONFIG_HOME/themes/$HALLWAYDE_THEME"
WALLBASH_DIRS=(
    "$XDG_CONFIG_HOME/wallbash"
    "$XDG_CONFIG_HOME/hallwayde/wallbash"
    "$XDG_DATA_HOME/wallbash"
    "$XDG_DATA_HOME/hallwayde/wallbash"
    "/usr/local/share/hallwayde/wallbash"
    "/usr/share/hallwayde/wallbash")
wallbashDirs=("${WALLBASH_DIRS[@]}")
export HALLWAYDE_THEME HALLWAYDE_THEME_DIR WALLBASH_DIRS wallbashDirs enableWallDcol
if [ -n "$HYPRLAND_INSTANCE_SIGNATURE" ]; then
    hypr_border="$(hyprctl -j getoption decoration:rounding | jq '.int')"
    hypr_width="$(hyprctl -j getoption general:border_size | jq '.int')"
fi
export hypr_border=${hypr_border:-${HALLWAYDE_BORDER_RADIUS:-2}}
export hypr_width=${hypr_width:-${HALLWAYDE_BORDER_WIDTH:-2}}
pkg_installed() {
    local pkgIn=$1
    if command -v "$pkgIn" &>/dev/null; then
        ...
        return 0
    elif hallwayde-shell pm.sh pq "$pkgIn" &>/dev/null; then
        return 0
    else
        return 1
    fi
...
set_conf() {
    local varName="$1"
    local varData="$2"
    touch "$XDG_STATE_HOME/hallwayde/staterc"
    if [ "$(grep -c "^$varName=" "$XDG_STATE_HOME/hallwayde/staterc")" -eq 1 ]; then
        sed -i "/^$varName=/c$varName=\"$varData\"" "$XDG_STATE_HOME/hallwayde/staterc"
    else
        echo "$varName=\"$varData\"" >>"$XDG_STATE_HOME/hallwayde/staterc"
    fi
}
set_hash() {
    local hashImage="$1"
...
check_package() {
    local lock_file="${XDG_RUNTIME_DIR:-/tmp}/hallwayde/__package.lock"
    mkdir -p "${XDG_RUNTIME_DIR:-/tmp}/hallwayde"
    if [ -f "$lock_file" ]; then
        return 0
    fi
    for pkg in "$@"; do
...
get_hyprConf() {
    local hyArg="$1"
    local file="${2:-"$HALLWAYDE_THEME_DIR/hypr.theme"}"
    local hyVar="$hyArg"
    local hyType=""
    ...
    if [ -z "$gsVal" ] || [[ $gsVal == \$* ]]; then
        ...
        *) grep "^[[:space:]]*\$default.$hyVar\s*=" \
            "XDG_DATA_HOME/hallwayde/hallwayde.conf" \
            "$XDG_DATA_HOME/hallwayde/hyprland.conf" \
            "/usr/local/share/hallwayde/hallwayde.conf" \
            "/usr/local/share/hallwayde/hyprland.conf" \
            "/usr/share/hallwayde/hallwayde.conf" \
            "/usr/share/hallwayde/hyprland.conf" 2>/dev/null | cut -d '=' -f2 | sed 's/^[[:space:]]*//;s/[[:space:]]*$//' | head -n 1 ;;
        esac
    else
        echo "$gsVal"
    fi
...
paste_string() {
    ...
    ignore_paste_file="$HALLWAYDE_STATE_HOME/ignore.paste"
...
accepted_mime_types() {
    ...
    for mime_type in "${mime_types_array[@]}"; do
        if file --mime-type -b "$file" | grep -q "^$mime_type"; then
            ...
            notify-send -u critical -a "HALLwayDE-Alert" "File type not supported for this wallpaper backend."
        fi
    done
}
dconf_write() {
...
export -f get_hyprConf get_rofi_pos is_hovered toml_write get_hashmap get_aurhlpr set_conf set_hash check_package get_themes print_log pkg_installed paste_string extract_thumbnail accepted_mime_types dconf_write send_notifs export_hallwayde_config
