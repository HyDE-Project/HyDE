...
Wall_Json() {
    setIndex=0
    [ ! -d "$HALLWAYDE_THEME_DIR" ] && echo "ERROR: \"$HALLWAYDE_THEME_DIR\" does not exist" && exit 0
    if [ -d "$HALLWAYDE_THEME_DIR/wallpapers" ]; then
        wallPathArray=("$HALLWAYDE_THEME_DIR/wallpapers")
    else
        wallPathArray=("$HALLWAYDE_THEME_DIR")
    fi
    wallPathArray+=("${WALLPAPER_CUSTOM_PATHS[@]}")
    [ -d "${XDG_PICTURES_DIR%/}/Wallpapers/${HALLWAYDE_THEME}" ] && wallPathArray+=("${XDG_PICTURES_DIR%/}/Wallpapers/${HALLWAYDE_THEME}")
    get_hashmap "${wallPathArray[@]}"
    wallListJson=$(printf '%s\n' "${wallList[@]}" | jq -R . | jq -s .)
    wallHashJson=$(printf '%s\n' "${wallHash[@]}" | jq -R . | jq -s .)
    jq -n --argjson wallList "$wallListJson" --argjson wallHash "$wallHashJson" --arg cacheHome "${HALLWAYDE_CACHE_HOME:-$HOME/.cache/hallwayde}" '
...
Wall_Cache() {
    ...
    if [ "$set_as_global" == "true" ]; then
        ...
        "$LIB_DIR/hallwayde/wallpaper/cache.sh" commence -w "${wallList[setIndex]}" &> /dev/null
        "$LIB_DIR/hallwayde/color.set.sh" "${wallList[setIndex]}" &
