...
wallpaper_cache_bootstrap() {
	if [[ ${HALLWAYDE_SHELL_INIT:-0} -ne 1 ]]; then
...
wallpaper_cache_init() {
	[ -d "$HALLWAYDE_THEME_DIR" ] && cacheIn="$HALLWAYDE_THEME_DIR" || {
		echo "Error: HALLWAYDE_THEME_DIR not found!"
		return 1
	}
...
fn_wallcache() {
	...
	if [ "$is_video" -eq 1 ]; then
		if [ ! -e "$thmbDir/$x_hash.thmb" ] || [ ! -e "$thmbDir/$x_hash.sqre" ] || [ ! -e "$thmbDir/$x_hash.blur" ] || [ ! -e "$thmbDir/$x_hash.quad" ] || [ ! -e "$dcolDir/$x_hash.dcol" ]; then
			...
			notify-send -a "HALLwayDE wallpaper" "Extracting thumbnail from video wallpaper..."
...
fn_envar_cache() {
	if command -v rofi &> /dev/null; then
		if [[ ! $XDG_DATA_DIRS =~ share/hallwayde ]]; then
			mkdir -p "$XDG_DATA_HOME/rofi/themes"
			ln -snf "$XDG_DATA_HOME/hallwayde/rofi/themes"/* "$XDG_DATA_HOME/rofi/themes/"
		fi
	fi
}
	...
	while getopts "w:t:f" option; do
		case $option in
			w)
				...
				;;
			t)
				cacheIn="$(dirname "$HALLWAYDE_THEME_DIR")/$OPTARG"
				if [ ! -d "$cacheIn" ]; then
				...
				;;
			f)
				cacheIn="$(dirname "$HALLWAYDE_THEME_DIR")"
				mode="_force"
				;;
			*)
				echo "... invalid option ..."
