...
source "$SHARE_DIR/doorwayde/env-theme"
dconf_populate() {
    cat << EOF
...
if [[ -r $HYPRLAND_CONFIG ]] && command -v "hyq" &> /dev/null; then
    eval "$(hyq "$HYPRLAND_CONFIG" --source --export env \
        -Q 'doorwayde:gtk-theme' \
        -Q 'doorwayde:color-scheme' \
        -Q 'doorwayde:icon-theme' \
        -Q 'doorwayde:cursor-theme' \
        -Q 'doorwayde:cursor-size' \
        -Q 'doorwayde:terminal' \
        -Q 'doorwayde:font' \
        -Q 'doorwayde:font-size' \
        -Q 'doorwayde:document-font' \
        -Q 'doorwayde:document-font-size' \
        -Q 'doorwayde:monospace-font' \
        -Q 'doorwayde:monospace-font-size' \
        -Q 'doorwayde:button-layout' \
        -Q 'doorwayde:font-antialiasing' \
        -Q 'doorwayde:font-hinting')"
    GTK_THEME=${_doorwayde_gtk_theme:-$GTK_THEME}
    COLOR_SCHEME=${_doorwayde_color_scheme:-$COLOR_SCHEME}
    ICON_THEME=${_doorwayde_icon_theme:-$ICON_THEME}
    CURSOR_THEME=${_doorwayde_cursor_theme:-$CURSOR_THEME}
    CURSOR_SIZE=${_doorwayde_cursor_size:-$CURSOR_SIZE}
    TERMINAL=${_doorwayde_terminal:-$TERMINAL}
    FONT=${_doorwayde_font:-$FONT}
    FONT_SIZE=${_doorwayde_font_size:-$FONT_SIZE}
    DOCUMENT_FONT=${_doorwayde_document_font:-$DOCUMENT_FONT}
    DOCUMENT_FONT_SIZE=${_doorwayde_document_font_size:-$DOCUMENT_FONT_SIZE}
    MONOSPACE_FONT=${_doorwayde_monospace_font:-$MONOSPACE_FONT}
    MONOSPACE_FONT_SIZE=${_doorwayde_monospace_font_size:-$MONOSPACE_FONT_SIZE}
    BUTTON_LAYOUT=${_doorwayde_button_layout:-$BUTTON_LAYOUT}
    FONT_ANTIALIASING=${_doorwayde_font_antialiasing:-$FONT_ANTIALIASING}
    FONT_HINTING=${_doorwayde_font_hinting:-$FONT_HINTING}
fi
...
DCONF_FILE="${XDG_CACHE_HOME:-$HOME/.cache}/doorwayde/dconf"
{
