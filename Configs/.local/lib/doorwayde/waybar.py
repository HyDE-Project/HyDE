...
STATE_FILE = Path(os.path.join(str(xdg_state_home()), "doorwayde", "staterc"))
DOORWAYDE_CONFIG = Path(os.path.join(str(xdg_state_home()), "doorwayde", "config"))
UNIT_NAME = f"doorwayde-{os.environ.get('XDG_SESSION_DESKTOP', 'unknown')}-bar.service"
...
def get_config_value(key, default=None):
    ...
    if DOORWAYDE_CONFIG.exists():
        with open(DOORWAYDE_CONFIG, "r") as file:
            for line in file:
...
def write_style_file(style_filepath, source_filepath):
    ...
    wallbash_gtk_css_file = os.path.join(str(xdg_cache_home()), "doorwayde", "wallbash", "gtk.css")
...
def get_value_from_hypr_theme(variable_name):
    ...
    if STATE_FILE.exists():
        try:
            with open(STATE_FILE, "r") as file:
                for line in file:
                    if line.startswith("DOORWAYDE_THEME="):
    ...
    theme_dir = os.path.join(str(xdg_config_home()), "doorwayde", "themes", theme_name)
...
def update_style(style_path):
    ...
    if not os.path.exists(theme_style_filepath):
        logger.error(
            f"Missing '{theme_style_filepath}', Please run 'doorwayde-shell reload' to generate it."
        )
...
def main():
    ...
    source_env_file(os.path.join(str(xdg_runtime_dir()), "doorwayde", "environment"))
    source_env_file(os.path.join(str(xdg_state_home()), "doorwayde", "config"))
