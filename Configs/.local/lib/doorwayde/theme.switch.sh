...
[ -z "$DOORWAYDE_THEME" ] && echo "ERROR: unable to detect theme" && exit 1
get_themes
confDir="${XDG_CONFIG_HOME:-$HOME/.config}"
Theme_Change() {
    local x_switch=$1
    for i in "${!thmList[@]}"; do
        if [ "${thmList[i]}" == "$DOORWAYDE_THEME" ]; then
            if [ "$x_switch" == 'n' ]; then
...
[[ ! " ${thmList[*]} " =~ " $themeSet " ]] && themeSet="$DOORWAYDE_THEME"
set_conf "DOORWAYDE_THEME" "$themeSet"
print_log -sec "theme" -stat "apply" "$themeSet"
export reload_flag=1
source "$LIB_DIR/doorwayde/globalcontrol.sh"
...
if [[ -r $HYPRLAND_CONFIG ]]; then
    ...
    [[ -r "$DOORWAYDE_THEME_DIR/hypr.theme" ]] && sanitize_hypr_theme "$DOORWAYDE_THEME_DIR/hypr.theme" "$XDG_CONFIG_HOME/hypr/themes/theme.conf"
    load_hypr_variables "$DOORWAYDE_THEME_DIR/hypr.theme"
    load_hypr_variables "${XDG_STATE_DIR:-$HOME/.local/state}/doorwayde/hyprland.conf"
fi
show_theme_status
...
[[ -d "$DOORWAYDE_CACHE_HOME/wallpapers/" ]] && find -H "$DOORWAYDE_CACHE_HOME/wallpapers" -name "*.png" -exec sh -c '
    for file; do
        base=$(basename "$file" .png)
        if pkg_installed ${base}; then
            "${LIB_DIR}/doorwayde/wallpaper.sh" --link --backend "${base}"
        fi
    done
' sh {} + &
if [ "$quiet" = true ]; then
    "$LIB_DIR/doorwayde/wallpaper.sh" -s "$(readlink "$DOORWAYDE_THEME_DIR/wall.set")" --global >/dev/null 2>&1
else
    "$LIB_DIR/doorwayde/wallpaper.sh" -s "$(readlink "$DOORWAYDE_THEME_DIR/wall.set")" --global
fi
