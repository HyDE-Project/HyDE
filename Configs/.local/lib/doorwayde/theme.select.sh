...
selector_menu() {
    ...
    if [ -n "$RofiSel" ]; then
        ...
        notify-send -a "DOORwayDE Alert" -i "$rofiAssetDir/theme_style_$selectedStyle.png" "Style $selectedStyle applied..."
...
rofiSel=$(
    ...
    while [ $i -lt ${#thmList[@]} ]; do
        ...
        -select "$DOORWAYDE_THEME"
)
if [ -n "$rofiSel" ]; then
    "$LIB_DIR/doorwayde/theme.switch.sh" -s "$rofiSel"
    notify-send -a "DOORwayDE Alert" -i "$iconsDir/Wallbash-Icon/doorwayde.png" " $rofiSel"
fi
