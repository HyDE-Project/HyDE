...
REPO_URL = "https://github.com/HyDE-Project/doorwayde-gallery.git"
CLONE_DIR = os.path.join(
    os.getenv("XDG_CACHE_HOME", os.path.expanduser("~/.cache")),
    "doorwayde/gallery-database",
)
JSON_DATA = None
...
def fetch_data():
    global JSON_DATA
    json_file_path = os.path.join(CLONE_DIR, "doorwayde-themes.json")
...
def get_theme_preview(theme):
    ...
    if theme == "[CONFIRM]":
        ...
        preview_text += (
            "👋 Let's DOORwayDErate your system with more cool themes!\n\n"
            "  [TAB] to mark a theme\n"
...
def fzf_menu():
    try:
        ...
        subprocess.run(["doorwayde-shell", "reload"], check=True)
...
def fetch_all_themes():
    ...
    themes_dir = os.path.join(config_home, "doorwayde/themes")
...
def main():
    parser = argparse.ArgumentParser(
        description="Imports themes from doorwayde-gallery repository",
        epilog="Env:\n"
    ...
    parser.add_argument(
        ...
        type=str,
        metavar="THEME",
        help="Fetch and update a specific theme by name (`all` to fetch all themes located in 'xdg_config/doorwayde/themes')",
    )
