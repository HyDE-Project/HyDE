...
function _dedup_zsh_plugins {
    ...
    # Load Plugins
    hallwayde_plugins=(git zsh-256color zsh-autosuggestions zsh-syntax-highlighting)
    plugins+=("${plugins[@]}" "${hallwayde_plugins[@]}")
    # Deduplicate plugins
    plugins=("${plugins[@]}")
    plugins=($(printf "%s\n" "${plugins[@]}" | sort -u))
...
function _load_deferred_plugin_system_by_hallwayde() {

    # Exit early if HALLWAYDE_ZSH_DEFER is not set to 1
    if [[ "${HALLWAYDE_ZSH_DEFER}" != "1" ]]; then
        unset -f _load_deferred_plugin_system_by_hallwayde
        return
    fi
...
function _load_compinit() {
    ...
    if [[ -n ${ZDOTDIR}/.zcompdump(#qN.mh+${HALLWAYDE_ZSH_COMPINIT_CHECK:-1}) ]]; then
        compinit
    else
        compinit -C
    fi
...
function _load_prompt() {
    ...
    if ! source ${ZDOTDIR}/prompt.zsh >/dev/null 2>&1; then
        [[ -f $ZDOTDIR/conf.d/hallwayde/prompt.zsh ]] && source $ZDOTDIR/conf.d/hallwayde/prompt.zsh
    fi
}
...
HALLWAYDE_ZSH_DEFER="1"      #Unset this variable in $ZDOTDIR/user.zsh to disable HALLwayDE's deferred Zsh loading.
HALLWAYDE_ZSH_PROMPT="1"     #Unset this variable in $ZDOTDIR/user.zsh to disable HALLwayDE's prompt customization.
HALLWAYDE_ZSH_NO_PLUGINS="0" #Set this variable to "1" in $ZDOTDIR/user.zsh to disable HALLwayDE's Zsh plugin loading.
...
# HALLwayDE Package Manager
PM_COMMAND=(hallwayde-shell pm)

# Optionally load user configuration // useful for customizing the shell without modifying the main file
if [[ -f $HOME/.hallwayde.zshrc ]]; then
    source $HOME/.hallwayde.zshrc # for backward compatibility
...
if [[ ${HALLWAYDE_ZSH_NO_PLUGINS} != "1" ]]; then
    _dedup_zsh_plugins
    if [[ "$HALLWAYDE_ZSH_OMZ_DEFER" == "1" ]] && [[ -r $ZSH/oh-my-zsh.sh ]]; then
        # Loads the buggy deferred oh-my-zsh plugin system by HALLwayDE // This is only for oh-my-zsh and compatibility
        _load_deferred_plugin_system_by_hallwayde
