...
_hallwayde_shell() {
    ...
    local built_in_commands hallwayde_scripts wallbash_scripts
    ...
    if (( $+commands[hallwayde-shell] )); then
        local scripts_raw
        scripts_raw=(${(f)"$(hallwayde-shell --list-script 2>/dev/null)"})
        hallwayde_scripts=(${scripts_raw[@]%.*})  # Remove extensions
    ...
    if [[ $CURRENT -eq 2 ]]; then
        ...
        local all_commands=($built_in_commands $hallwayde_scripts)
...
compdef _hallwayde_shell hallwayde-shell