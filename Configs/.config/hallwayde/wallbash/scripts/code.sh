...
# shellcheck source=$HOME/.local/bin/hallwayde-shell
...
cacheDir="${cacheDir:-$XDG_CACHE_HOME/hallwayde}"
...
for i in "${!codeConf[@]}"; do
    ...
    extTheme="$(
        "$LIB_DIR/hallwayde/parse.json.py" -CRQ '.["workbench.colorTheme"]' "${codeConf[i]}/User/settings.json"
    )"
    if [ "${extTheme}" != "${set_theme}" ]; then
        "$LIB_DIR/hallwayde/parse.json.py" -CU '.["workbench.colorTheme"]' "${set_theme}" "${codeConf[i]}/User/settings.json"
